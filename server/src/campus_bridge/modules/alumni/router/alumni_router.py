...
from campus_bridge.data.schemas.alumni import AlumniResponse, CreateAlumni, UpdateAlumniResponse, UpdateAlumni
from fastapi import APIRouter, Depends, status

from campus_bridge.api.v1.dependencies import get_current_user
from campus_bridge.data.models import User
from campus_bridge.modules.alumni.service.alumni_service import AlumniService, get_alumni_service
...
async def get_all_alumni(
    ...
    if current_user.role != RoleEnum.ADMIN:
        raise UnauthorizedError(
            obj="alumni",
            act="get_all_alumni_by_admin_only"
        )   
    return await alumni_service.get_all_alumni()

@router.get("/college", status_code=status.HTTP_200_OK, response_model=list[AlumniResponse])
async def get_all_alumni_by_college(
    ...
    # If college_id is provided but the user is not an Admin, check BOLA.
        ...
        raise UnauthorizedError(
            obj="alumni",
            act="view_alumni_of_other_college"
        )
...
async def create_alumni(
    ...
    # 2. Officials: Can only create alumni under their own college
        ...
        if alumni.college_id and alumni.college_id != current_user.college_id:
            raise UnauthorizedError(
                obj="alumni",
                act="create_alumni_in_other_college"
            )
    ...
    # 3. Alumni: Can only create their own profile under their own college
        ...
        if target_user_id != current_user.id:
            raise UnauthorizedError(
                obj="alumni",
                act="create_alumni_profile_for_another_user"
            )
        if alumni.college_id and alumni.college_id != current_user.college_id:
            raise UnauthorizedError(
                obj="alumni",
                act="create_alumni_profile_in_another_college"
            )
...
async def update_alumni(
    ...
    if current_user.role not in [RoleEnum.ADMIN, RoleEnum.OFFICIALS, RoleEnum.ALUMNI]:
        raise UnauthorizedError(
            obj="alumni",
            act="update_alumni_by_admin_officials_alumni_only"
        )
    # update alumni by id only for admin and officials
    if alumni_id and (current_user.role == RoleEnum.ADMIN or current_user.role == RoleEnum.OFFICIALS):
...
async def delete_alumni(
    ...
    if current_user.role not in [RoleEnum.ADMIN, RoleEnum.OFFICIALS, RoleEnum.ALUMNI]:
        raise UnauthorizedError(
            obj="alumni",
            act="delete_alumni_by_admin_officials_only"
        )
