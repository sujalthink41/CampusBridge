...
class User(
    Base, IdMixin, TableNameMixin, TimestampMixin, SoftDeleteMixin, VerifyAccount
):
    ...
    role: Mapped[RoleEnum] = mapped_column(role_enum, nullable=False, index=True)
    ...
    alumni_profile: Mapped["Alumni"] = relationship(
        "Alumni", back_populates="user", uselist=False
    )

    official_profile: Mapped["CollegeOfficial"] = relationship(
        "CollegeOfficial", back_populates="user", uselist=False
    )
